...
module.exports.create = async (event) => {
  const promise = new Promise((resolve) => {
    const { admin, start, finish, title, coverPhoto, visibility, users } = JSON.parse(
      event.body
    );
    if (users !== undefined) {
      createNewBoard(admin, start, finish, title, coverPhoto, visibility, users)
        .then((data) => {
          const response = {
            statusCode: 200,
            headers: {
