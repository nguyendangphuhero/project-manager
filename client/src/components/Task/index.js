...
class Task extends React.Component {
  ...
  submitsubtask = (subtask) =>
    new Promise((resolve, reject) => {
      this.setState(
        {
          subtask: [...this.state.subtask, subtask],
        },
        async () => {
    ...
    deleteSubtask = (subtaskId) => {
      ...
      for (let i = 0; i < subtask.length; i++) {
        const subtask1 = subtask[i];
        if (subtask1.id === subtaskId) {
          // remove id matched subtask
          subtask.splice(i, 1);
    ...
    editSubtask = (subtaskId, subtask1) => {
      let subtask = this.state.subtask;
      for (let i = 0; i < subtask.length; i++) {
        if (subtask[i].id === subtaskId) {
          // remove id matched comment
          subtask[i].text = subtask1;
          this.setState({ subtask: subtask }, () => {
            TaskHelpers.HandleTaskPropertyUpdate(
              this.context.renderedBoard,
              this.props.task.id,
  ...
  render() {
    ...
    let avatarCounter = 0;
    // console.log("DSAS", this.props.task);
    return (
      <Draggable draggableId={task.id} index={index}>
        {(provided, snapshot) => (
          ...
          >
            ...
            {/* truyen data sang edittaskmodal */}
            <EditTaskModal
              statusTask={this.state.status}
              ...
              editStatus={this.handleStatusChange}
              editStart={this.handleStartChange}
              editDatePicker={this.handleDatePickerChange}
